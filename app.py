...
st.title("Reversal Detection Pro v3.0 Scanne 🚀")
...
# Put Custom List first
index_options = ["Custom List"] + list(indices.keys())
selected_index = st.sidebar.selectbox("Select Index", index_options)
...
if st.button("Run Scan"):
    ...
    if not symbols:
        ...
        # Execute Scan
        # We manually handle futures to update progress bar
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_symbol):
                ...
                status_text.text(f"Scanning {completed} of {total_symbols} symbols... ({sym})")
